...
Esistono fenomeni in cui rapidità di variazione di una certa quantità è proporzionale, in ogni istante, alla quantità stessa.
...
In particolare, posto di saper determinare esplicitamente la primitiva di $a$ l'inversa della primitiva di $\frac{1}{f}$\\ (condizione talvolta piuttosto gravosa), questi problemi sono di semplice risoluzione.
...
\begin{theorem}
    Sia $I$ intervallo di $\R$, sia $a:I \to \K$ una funzione continua; e siano $t_0\in I$ e $y_0\in\K$ fissati.\\ Esiste un'unica funzione $\phi\in C^1$ tale che 
...
({\color{red} Meglio rimpiazzare $\K$ con $\R$, sia perché non abbiamo definito l'integrazione su un campo generico (non è neppure detto che abbia senso), sia perché non abbiamo definito l'esponenziale su un campo generico.})
...
\color{red}{La verifica che 'sta roba funzioni è semplice, ma qual è l'idea per arrivare a questa espressione esplicita?\\ Non pretenderemo che le persone la ricordino a memoria...}
\end{theorem}
\subsection{Esempi}
