...
\begin{itemize}
    ...
    \item $||\lambda x||=|\lambda|\cdot||x||\qquad \forall\lambda\in\R,\; \forallx\in V$
...
Siano $(X,d_X)$ e $(Y,d_Y)$ due spazi metrici e sia $f:X\to Y$ una funzione. Diremo che $f$ è lipschitziana se esiste una costante $L$ tale che: $$d_Y(f(x),f(y))\leq Ld_X(x,y)\qquad \forall x,y\in X$$    
\end{definizione}
...
$$d(f(x),f(y))<Ld(x,y)\qquad\forall x,y\in X$$
...
\begin{definizione}[Insieme Compatto]
    Un sottoinsime $K$ dello spazione metrico $(X,d)$ si dice compatto se da ogni successione $x_k$ di punti di $K$ se ne può estrarre una convergente verso un punto $x\in K$
\end{definizione}
...
\end{theorem}
\begin{proof}
 Se $K$ è chiuso e limitato, sia $x_k=(x_k1,x_k2,\dots,x_kn)$ una successione di puntidi $K$. le successioni $x_ki$ sono successioni di reali limtate. Per il teorema di Bolzano-Weierstrass dalla successione $x_k$ se ne può estrarre una avente la prima coordinata convergente; da questa un altra con seconda coordinata convergente, e così via fino ad $n$. Si ottiene così una successione srettamente crescente $k_h$ di numeri naturali tale che  $x_{k_h,i}\to x_i$ per ogni $i=2,3,\dots,n$. Pertanto $x_k\to x=(x_1,x_2,\dots,x_n)$.\\
 Essendo $K$ chiuso ne segue che $x\in K$ e quindi $K$ è compatto.\\ Viceversa se $K$ è compatto esso è chiuso per la proposizione precedente. Se $K$ non fosse limitato, esisterebbe una successione $x_k$ di punti di $K$ tale che: $$\lim_{k\to\infty}|x_k|=+\infty$$ e da questa non si potrebbe estrarre alcuna successione convergente, contro l'ipotesi che $K$ sia compatto.
\end{proof}
...
\subsubsection{L'Oscillatore Armonico}
...
\subsection{Metodi di Risoluzione Eq. non omogenee}
...
\textbf{J46}$(\star\star)$ Si dimostri che per qualunque $f:\mathbb{R}^+\to\mathbb{R}^+$ continua e strettamente crescente si ha che l'integrale\\ $\int_{0}^{+\infty}\sin(x\cdot f(x))\,dx $ è convergente nel senso di Riemann improprio.
