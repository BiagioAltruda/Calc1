...
\begin{theorem}[Taylor]
 Siano $I$ un Intervallo, $x_0\in I$ e $f:I\to\R$.\\
 $d=0,1,...;$\\
 $f$ derivabile $d-1$ volte su $I$ e $d$ volte in $x_0$
 Si definisce quindi $P_d(h)$ il \textbf{polinomio di Taylor} di $f$ in $x_0$ di grado $d$:
 $$P_d(h)\sum_{n\geq 0}^{}\frac{f^{(n)}(x_0)}{n!}(x-x_0)^n.$$
E sia inoltre $R_d$ un \textbf{resto}, tale che $f(x_0+h)=P_d(h) + R_d(h)$. \\

Ci sono due tipi di resto che verificano il teorema, quelli di Peano e Lagrange. Assumiamo le ipotesi più forti ed enunciamo la formula integrale del resto di Lagrange. \\

Sia $f\in C^{d+1}$ in $I$ allora
$$R_d(h)=\frac{1}{d!}\int_{0}^{h}(h-t)^d f^{(d+1)}(x_0+t)dt$$
\end{theorem}
\begin{proof}
    Imposto un'induzione su $d$, il grado del Polinomio di Taylor.\\
    Voglio dimostrare che: 
    $$f(h)=P_d(h)+R_d(h)= P_d(h)+\frac{1}{d!}\int_{0}^{h}(h-t)^d f^{(d+1)}(x_0+t)dt$$
    per $d=0$: $\star_1$
    $$f(h)=f(0)+\int_{0}^{h}f'(t)dt$$
    vero per il \emph{T.F.C.I.}.
 svolgo un passaggio in più e provo con $d=1$:$\star_2$\\
 Integrando l'espressione di sopra per parti ottengo:
 $$f(h)=f(0)+\int_{0}^{h}f(t)\cdot 1 dt=f(0)+\left|(h-t)f'(t)\right|_0^h + \int_{0}^{h}(h-t)f''(t)dt=f(0)+f'(0)h+\int_{0}^{h}(h-t)f''(t)dt$$
 Ora, col passo induttivo, derivo $\star_d$ da $\star_{d-1}:$
 $$f(h)=P_{d-1}(h)+\frac{1}{(d-1)!}\int_{0}^{h}(h-t)^{d-1}f^{(d)}(h)dt = P_{d-1}(h)+\frac{1}{(d-1)!}\left|-\frac{(h-t)^d}{d}f^{(d)}(h)\right|_0^h-\frac{1}{d!}\int_{0}^{h}-(h-t)^df^{(d+1)}(h)dt=$$
 $$=P_{d-1}(h)+\frac{1}{d!}f^{d}(0)h^d$$
\end{proof}
[controllare questa cosa \emph{abominevole}]
...
\begin{itemize}
    \item $\sqrt{a^2-x^2}$
    \qquad Porre $x=a\sin{t}$, Infatti $\sqrt{a^2-x^2}=a\cos{t}$
        \item $\sqrt{x^2-a^2}$
    \qquad Porre $x=a\frac{1}{\cos{t}}$, Infatti $\sqrt{x^2-a^2}=a\tan{t}$
        \item $\sqrt{a^2+x^2}$
    \qquad Porre $x=a\tan{t}$, Infatti $\sqrt{a^2+x^2}=\frac{a}{\cos{t}}$
\end{itemize}
Se ad esempio volessimo integrare $$\int{\frac{\sqrt{x^2-1}}{x^2} dx}$$
Utilizziamo la sostituzione $x=\tan{t}$, e $dx=\frac{dt}{\cos^2{t}}$
$$\int{\frac{\sqrt{x^2-1}}{x^2} dx}=\int{\frac{\sqrt{\tan^2{t}+1}}{\tan^2{t}}}\frac{dt}{\cos^2{t}}=\int{\frac{dt}{\sin^2{t}\cos{t}}}=\int{\frac{\sin^2{t}+\cos^2{t}}{\sin^2{t}\cos{t}}}=$$ $$\int{\frac{dt}{\cos{t}}}+\int{\frac{\cos{t}}{\sin^2{t}}}=\log\bigg|{ \tan{t}+\frac{1}{\cos{t}}\bigg| +c=$$
$$=\log\bigg|x+\sqrt{x^2+1}{\bigg|}-\frac{\sqrt{x^2+1}}{x}+C$$
\subsection{Integrazione per Parti}
Se $u=\phi(x)$ e $v=\psi(x)$ sono derivabili allora si ha che: $$\int u\;dv = uv - \int v\;du$$
